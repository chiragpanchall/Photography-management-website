...
if ($_REQUEST['action']) 
{
	...
	$Offer_Begins = $_POST['Offer_Begins'];
	$Offer_Ending = $_POST['Offer_Ending'];
	$OffPer = $_POST['OffPercentage'];
	...
	switch ($action) {
		case 'insert':
			...
			echo $insert = "INSERT INTO `offer` (`Offercode`,`Product_Off_Percentage`,`Offer_Details`,`Offer_Begins`,`Offer_Ending`) VALUES ('".$Offercode."','".$OffPer."','".$Offer_Details."','".$Offer_Begins."','".$Offer_Ending."') ";
			...
			if ($qry) 
			{
				...
				header("location:index.php?page=offer_list");
				echo "Insert Okok";
			}
			else
			{
...
/* fname lname user userid password photo pah email*/
			...
			case 'delete':
			$admin_id = $_REQUEST['idOffer'];
			// Upload Folder In Image Delete Code //
		 
			$delete = "DELETE FROM `offer` WHERE `idOffer` = '".$admin_id."' ";
